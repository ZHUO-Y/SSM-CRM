...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            //动态绑定复选框。
            $("#activityBody").on("click", $(":checkbox[name=someCheck]"), clickSome)
            //页面加载完毕以后，展现市场活动信息列表。
            showActivityList();
            $("#bigCheck").click(clickBig);
        });

        //点击关联市场活动的模态窗口中的全选框按钮。
        function clickBig() {
            $(":checkbox[name=someCheck]").prop("checked", this.checked);
        }

        //点击关联市场活动的模态窗口中的复选框按钮。
        function clickSome() {
            $("#bigCheck").prop("checked", $(":checkbox[name=someCheck]").length ===
                $(":checked[name=someCheck]").length);
        }
        ...
        function showActivityList() {
            $.ajax({
                ...
                data: {
                    //需要传入clue id，查到activityid然后查到activity返回
                    "id": "${c.id}"
                },
                type: "get",
                dataType: "json",
        ...
        function unbund(id) {
            ...
            //可以根据市场活动id，删除关联表的相关记录，但是不规范。最好传入的id是关联表的id。所以最好返回市场活动activity的时候，id为关联记录的id。

        }

        function searchActivityList(event) {
            if (event.keyCode == 13) {
                // alert("回车")
                //展示市场活动列表
        ...
        function showActivityListByName() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (i, n) {
                        html += '<tr>'
                        html += '<td><input type="checkbox" name="someCheck" value="' +
                            n.id + '" /></td>'
                        html += '<td>' + n.name + '</td>'
                        html += '<td>' + n.startDate + '</td>'
                        html += '<td>' + n.endDate + '</td>'
        ...
        function bund() {
            //需要传入clueId和activityId
            var $xz = $(":checked[name=someCheck]");
            if($xz.length === 0){
                alert("请选择至少一条需要关联的市场活动");
            }else {
    ...
    <div class="modal-dialog" role="document" style="width: 80%;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-inline" role="form">
                        <div class="form-group has-feedback">
                            <input type="text" class="form-control" id="searchName"
                                   style="width: 300px;"
                                   onkeydown="searchActivityList(event)"
                ...
                <table id="activityTable" class="table table-hover"
                    ...
                    <tr style="color: #B3B3B3;">
                        <td><input type="checkbox" id="bigCheck" <%--onclick="clickBig()"--%>
                        /></td>
                        <td>名称</td>
                        <td>开始日期</td>
                        <td>结束日期</td>
                    ...
                    <%--
                    <tr>        关联市场活动需要用到市场活动id和线索id（放在session里。）
                        <td><input type="checkbox" name="someCheck" id="市场活动id" /></td>
                        <td>发传单</td>
                        <td>2020-10-10</td>
                        <td>2020-10-20</td>
                        <td>zhangsan</td>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="bund()">关联
                </button>
            </div>
        </div>
    </div>
...
<div>
    <div style="position: relative; top: 60px; left: 40px;">
        ...
        <div>
            <a onclick="clickbund()"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
        </div>
    </div>
</div>
